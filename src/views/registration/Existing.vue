    ...
    <v-navigation-drawer v-model="nav" hide-overlay width="300" app>
      ...
      <v-stepper v-model="e1" vertical>
        ...
        <v-stepper-step :complete="e1 > 2" step="2">
          ...
          <span
            class="caption font-weight-thin pa-1"
          >We need the Name and the Owner's Name of the registered establishment.</span>
        </v-stepper-step>
        <v-divider></v-divider>
              ...
              <v-stepper-content step="4">
                <v-card-text>
                  <v-form ref="step4">
                    <v-layout row wrap>
                      <v-flex xs12>
                        <v-text-field outline label="Username" v-model="account.username"></v-text-field>
                      </v-flex>
                      <v-flex xs12>
                        <v-text-field
                          outline
                ...
                <v-card-text>
                  <v-layout row wrap>
                    <v-flex xs12>
                      ...
                      <div
                        class="pa-2 title font-weight-thin"
                      >We have sent you an email confirmation. Please check your email to confirm you account.</div>
                    </v-flex>
                  </v-layout>
                </v-card-text>
              </v-stepper-content>
    ...
    next() {
      ...
      switch (this.e1) {
        ...
        case 2:
          ...
          break;
        case 3:
          this.validateExisting().then(result => {
            if (result) {
              this.e1++;
            }
          });
    ...
    validateExisting() {
      return new Promise((resolve, reject) => {
        if (this.$refs.step3.validate()) {
          ...
          this.$store
            .dispatch("VERIFY_LICENSES", this.form)
            .then(result => {
              ...
              if (result.data.model) {
                ...
                this.$notifyError([
                  {
                    message: "License not found! Please check your details",
                    color: "warning",
                    icon: "error_outline"
                  }
                ]);
    ...
    saveExisting() {
      return new Promise((resolve, reject) => {
        if (this.$refs.step4.validate()) {
          ...
          this.$store
            .dispatch("EXISTING_LIC_REG", reg_details)
            .then(result => {
              this.isLoading = false;
