<template>
    <v-layout row wrap v-if="success_confirm">        
        ...
        <v-flex xs12 pa-4>
        <span class="headline font-weight-thin">Your account <b>({{account_name}})</b> is already confirmed. Please wait while we process your application. </span>                               
        </v-flex>
        <v-flex xs12 pa-4>
        <span class="headline font-weight-thin">Account Approval will take 1-2 working days. Expect an email in <b>{{email}}</b> notifying you of the activation status.</span> 
        </v-flex>
        <v-flex xs12 pa-4>
    ...
    <v-layout row wrap v-else>
        ...
        <v-flex xs12 pa-4>
        <span class="headline font-weight-thin">Could not verify your account. It's seems that the confirmation email is already expired.</span>                       
        </v-flex>
         <v-flex xs12 pa-4>
...
export default {
    ...
    methods:{
        init(){
            this.$store.dispatch('CONFIRM', this.$route.query.key)
            .then(res=>{
